...
type QR = { numero_mesa: number; url: string; tipo?: "mesa" | "karaoke"; etiqueta?: string };
...
export function QRsPanel({ accessToken }: Props) {
  ...
  return (
    <div className="panel">
      ...
      <p className="panel-sub">Imprime uno por zona. Al escanearlo, el cliente entra directo a esa mesa o al box.</p>
      {error && <div className="error-msg">{error}</div>}
      <div className="qr-grid">
        {qrs.map((q) => {
          const nombre = q.etiqueta ?? `Mesa ${q.numero_mesa}`;
          return (
            <div key={q.numero_mesa} className={`qr-card ${q.tipo === "karaoke" ? "qr-box" : ""}`}>
